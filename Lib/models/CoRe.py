...
class DictBlock(nn.Module):
    ...
    def __init__(self, n_channel, dict_size, mu=0.0, lmbd=0.0, n_dict=1, non_negative=True,  # model parameters
                 ...
                 n_steps=10, step_size_fixed=True, FISTA=False, step_size=0.1, w_norm=True, padding_mode="constant"):  # training parameters
        ...
        #self.weight1 = nn.Parameter(torch.Tensor(dict_size, self.n_dict * n_channel, kernel_size, kernel_size))
        #self.w = nn.Parameter(torch.Tensor(dict_size, self.n_dict * n_channel, 1, 1))
        with torch.no_grad():
    ...
    def fista_forward(self, x):
        ...
        for i in range(self.n_steps):
            ...
            if i == 0:
                ...
                xp = F.conv_transpose2d(c, weight, padding=1)
                ...
                c = step_size * F.conv2d(c, weight, bias=None, stride=self.stride,
                                         padding=self.padding)
                ...
                c_pre = c
                # weight = self.normalize(weight)
                c = step_size * F.conv2d(a, weight, bias=None, stride=self.stride,
                                         padding=self.padding)
                xp = F.conv_transpose2d(c, weight, bias=None, stride=self.stride, padding=self.padding,
                                        output_padding=self.conv_transpose_output_padding)
                r = c - xp
    ...
    def ista_forward(self, x):
        ...
        for i in range(self.n_steps):
            ...
            step_size = self.step_size[index]
            if i == 0:
                c = step_size*F.conv2d(x.repeat(1, self.n_dict, 1, 1),weight,padding=self.padding)
                #print(c.shape)
                #c = step_size* x + c
                xp = F.conv_transpose2d(c, weight, stride=self.stride, padding=self.padding,
                                        output_padding=self.conv_transpose_output_padding)
                #print(xp.shape)
                #xp = F.conv2d(c,dweight,padding=1, bias=None)
                #print(z.shape)
                # if xp==z:
                #     print('1')
                # else:
                ...
                a = step_size * F.conv2d(x, weight, padding=1, bias=None)
                z = step_size * F.conv2d(c,weight,padding=1, bias=None)
                xp = F.conv_transpose2d(z, weight, bias=None, stride=self.stride, padding=self.padding,
                                        output_padding=self.conv_transpose_output_padding)
                #print(c.shape)
                r = a - xp
            #print(r.shape,c.shape,x.shape)
                c = r + c
            c = self.nonlinear(c)
            ...
            # c = F.conv_transpose2d(c, weight, bias=None, stride=self.stride, padding=self.padding,
            #                            output_padding=self.conv_transpose_output_padding)
            #c = x-c
